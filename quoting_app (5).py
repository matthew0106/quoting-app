...
# --- Product Selection ---
try:
    product_files = [f.replace("_materials.xlsx", "") for f in os.listdir(MATERIALS_DIR) if f.endswith("_materials.xlsx")]
except FileNotFoundError:
    st.error("Materials folder not found. Please ensure 'materials/' exists.")
    st.stop()
...
try:
    ...
    st.error(f"Missing materials or employee file for product: {product_code}")
    st.stop()
...
# Each row should have: UNIT COST and USAGE PER UNIT
...
# --- Other Costs ---
contingency_cost = 0.20 * total_material_cost
storage_cost = 0.10 * quantity_ordered

# --- Total Calculation ---
raw_total = (
    total_labour_cost + total_material_cost +
    contingency_cost + storage_cost + freight_cost
)
final_quote = raw_total * (1 + margin_rate / 100)
price_per_item = final_quote / team_daily_output
...
st.markdown(f"**Contingency (20% of materials):** ${contingency_cost:,.2f}")
